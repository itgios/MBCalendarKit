...
- (void)testDayMonthYearCalendarComponentsWithGregorianCalendar
{
    ...
    XCTAssertEqual(day, 23, @"Day: %li",(long)day);
    XCTAssertEqual(month, 10, @"Month: %li", (long)month);
    XCTAssertEqual(year, 1991, @"Year:%li", (long)year);
}
...
- (void)testDayMonthYearCalendarComponentsWithHebrewCalendar
{
    ...
    XCTAssertEqual(day, 1, @"Day: %li", (long)day);
    XCTAssertEqual(month, 1, @"Month: %li", (long)month);
    XCTAssertEqual(year, 5773, @"Year:%li", (long)year);
}
...
- (void)testDaysPerMonthForStandardGregorianYear
{
    ...
    for(NSUInteger month = 0; month < kMonthsPerGregorianYear; month++){
        ...
        XCTAssertEqual(assumedResult, daysPerMonth, @"Month: %lu", month+1);
    }
}
...
- (void)testWeeksPerMonthForGregorianYear2013
{
    ...
    for(NSUInteger month = 0; month < kMonthsPerGregorianYear; month++){
        ...
        XCTAssertEqual(assumedResult, daysPerMonth, @"Month: %lu, Assumed: %lu Actual: %lu", (unsigned long)month, (unsigned long)assumedResult, (unsigned long)daysPerMonth);
    }
...
- (void)testDaysPerGregorianYear2010
{
    ...
    XCTAssertEqual(assumedResult, actualResult, @"Assumed: %lu Actual: %lu", (unsigned long)assumedResult, (unsigned long)actualResult);
}
